---
title: "Streamlit 1.53.0 is here! 🎉"
...
![Streamlit 1.53.0 release notes hero](/app/static/articles/2026-01-14-1-53-0-release-notes/hero.png)

Streamlit 1.53.0 is out. If you’re building on Snowflake, juggling per-user state, or dreaming of running Streamlit inside a bigger ASGI stack, this one’s for you. 🎈

**TL;DR**

- 🌐 Starlette server mode + ASGI entry point (`st.App`) (experimental)
- 🔑 OIDC access tokens can be exposed via `st.user.tokens`
- 💾 Session-scoped `st.cache_data` / `st.cache_resource` + cleanup via `on_release`
- ❄️ Caller’s rights connections for Streamlit in Snowflake

Want the full list? Jump to the official [release notes](https://docs.streamlit.io/develop/quick-reference/release-notes).

## 🌐 ASGI experiments: Starlette mode + `st.App`
...
This unlocks the stuff you expect in the ASGI world: mounting Streamlit under FastAPI/Starlette, sharing middleware, and adding “boring but necessary” routes like health checks.

### Starlette mode

Long term, we’re planning to switch Streamlit apps to run on Starlette by default. For now, it’s **opt-in** while we harden compatibility.
...
`st.App` is the other half: an **ASGI-compatible entry point** so you can attach Streamlit to a broader app and add “real web app” affordances like routes and middleware. Since this is experimental, you need to import it via `from streamlit.starlette import App` for now.
...
If you’re using OIDC auth, you can now optionally expose an **access token** in `st.user.tokens`. This is handy when your app needs to call external APIs **on behalf of the logged-in user**.
...
## 💾 Session-scoped caching + cleanup hooks
...
Global caches are awesome… until your data is **user-specific**. Session-scoped caching gives you the same speedups while keeping cached results isolated per session. Great for:

- per-user preferences / feature flags
- per-user API calls where responses differ by identity
- resources you _don’t_ want shared across users (clients, connections, models with user context)
...
## ❄️ Caller’s rights connections

Caller’s rights means your app queries Snowflake **as the currently logged-in user**, not as a shared service user.

When you run Streamlit in Snowflake with caller’s rights enabled, you can create a **session-scoped** connection that uses the _current user’s_ identity.
...
That’s it for 1.53.0. If you try Starlette mode / `st.App`, please share bugs + feedback in the GitHub thread above.
