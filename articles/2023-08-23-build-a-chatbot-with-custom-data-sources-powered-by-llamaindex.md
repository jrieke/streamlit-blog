...
subtitle: "Augment any LLM with your own data in 43 lines of code!"
date: 2023-08-23
authors:
  - "Caroline Frasca"
category: "LLMs"
...
![Build a chatbot with custom data sources, powered by LlamaIndex](https://streamlit.ghost.io/content/images/size/w2000/2023/08/Community--2--3.svg)
...
Before we get started, let's walk through the basics of [LlamaIndex](https://gpt-index.readthedocs.io/en/latest/index.html?ref=streamlit.ghost.io).
...
![rag-with-llamaindex-1](https://streamlit.ghost.io/content/images/2023/08/rag-with-llamaindex-1.png#border)
...
No matter what your LLM data stack looks like, [LlamaIndex](https://www.llamaindex.ai/?ref=streamlit.ghost.io) and [LlamaHub](https://llamahub.ai/?ref=streamlit.ghost.io) likely already have an integration, and new integrations are added daily. [Integrations](https://gpt-index.readthedocs.io/en/stable/community/integrations.html?ref=streamlit.ghost.io#integrations) with LLM providers, vector stores, data loaders, evaluation providers, and agent tools are already built.

LlamaIndex's [Chat Engines](https://gpt-index.readthedocs.io/en/latest/core_modules/query_modules/chat_engines/root.html?ref=streamlit.ghost.io) pair nicely with Streamlit's chat elements, making building a contextually relevant chatbot fast and easy.
...
Want to jump right in? Check out [the app](https://llamaindex-chat-with-docs.streamlit.app/?ref=streamlit.ghost.io) and [the code](https://github.com/streamlit/llamaindex-chat-with-streamlit-docs/blob/main/streamlit_app.py?ref=streamlit.ghost.io).
...
![llamaindexgif](https://streamlit.ghost.io/content/images/2023/08/llamaindexgif.gif#browser)
...
This app will use GPT-3.5, so you'll also need an OpenAI API key. Follow our instructions [here](https://streamlit.ghost.io/langchain-tutorial-1-build-an-llm-powered-app-in-18-lines-of-code/#step-1-get-an-openai-api-key) if you don't already have one.
...
* If you plan to deploy this app on Streamlit Community Cloud, the following contents should be [added to your app's secrets via the Community Cloud modal](https://docs.streamlit.io/streamlit-community-cloud/deploy-your-app/secrets-management?ref=streamlit.ghost.io#deploy-an-app-and-set-up-secrets).
...
The [full app](https://github.com/streamlit/llamaindex-chat-with-streamlit-docs/blob/main/streamlit_app.py?ref=streamlit.ghost.io) is only 43 lines of code. Let's break down each section.
...
* Use [session state](https://docs.streamlit.io/library/api-reference/session-state?ref=streamlit.ghost.io) to keep track of your chatbot's message history.
...
Download the markdown files for Streamlit's documentation from the [`data`](https://github.com/streamlit/llamaindex-chat-with-streamlit-docs/tree/main/data?ref=streamlit.ghost.io) demo app's GitHub repository folder. Add the `data` folder to the root level of your app. Alternatively, add your data.

🎈

If you’re running your app locally, check out LlamaIndex’s library of data connectors, available via [LlamaHub](https://llamahub.ai/?ref=streamlit.ghost.io), which makes it fast and easy to retrieve data from a variety of sources (including [GitHub repositories](https://llamahub.ai/l/github_repo?ref=streamlit.ghost.io)).

Define a function called `load_data()`, which will:

* Use LlamaIndex’s [`SimpleDirectoryReader`](https://gpt-index.readthedocs.io/en/stable/examples/data_connectors/simple_directory_reader.html?ref=streamlit.ghost.io#simple-directory-reader) to passLlamaIndex's the folder where you’ve stored your data (in this case, it’s called `data` and sits at the base level of your repository).
...
* Construct an instance of LlamaIndex’s [`ServiceContext`](https://gpt-index.readthedocs.io/en/latest/core_modules/supporting_modules/service_context.html?ref=streamlit.ghost.io), whichLlamaIndex'stion of resources used during a RAG pipeline's indexing and querying stages.
* `ServiceContext` allows us to adjust settings such as the LLM and embedding model used.
* Use LlamaIndex’s [`VectorStoreIndex`](https://gpt-index.readthedocs.io/en/stable/core_modules/data_modules/index/vector_store_guide.html?ref=streamlit.ghost.io) to creaLlamaIndex'sory `SimpleVectorStore`, which will structure your data in a way that helps your model quickly retrieve context from your data. Learn more about LlamaIndex’s `Indices` [here](https://gpt-index.readthedocs.io/en/stable/core_modules/data_modules/index/root.html?ref=streamlit.ghost.io#indexes). This function returns the `VectorStoreIndex` object.
...
1. [**Condense question engine**](https://gpt-index.readthedocs.io/en/stable/examples/chat_engine/chat_engine_condense_question.html?ref=streamlit.ghost.io): Always queries the knowledge base. Can have trouble with meta questions like “What did I previously ask you?”
2. [**Context chat engin"**](https://gpt-index.readthedocs.io/en/stable/examples/chat_engine/chat_engine_context.html?ref=streamlit.ghost.io): Always queries the knowledge base and uses retrieved text from the knowledge base as context for following queries. The retrieved context from previous queries can take up much of the available context for the current query.
3. [**ReAct agent**](https://gpt-index.readthedocs.io/en/latest/examples/chat_engine/chat_engine_react.html?ref=streamlit.ghost.io): Chooses whether to query the knowledge base or not. Its performance is more dependent on the quality of the LLM. You may need to coerce the chat engine to correctly choose whether to query the knowledge base.
4. [**OpenAI agent**](https://gpt-index.readthedocs.io/en/latest/examples/chat_engine/chat_engine_openai.html?ref=streamlit.ghost.io): Chooses whether to query the knowledge base or not—similar to ReAct agent mode, but uses [OpenAI’s built-in fuOpenAI'salling capabilities](https://platform.openai.com/docs/guides/gpt/function-calling?ref=streamlit.ghost.io).
...
2. Navigate to [Streamlit Community Cloud](https://share.streamlit.io/?ref=streamlit.ghost.io), click `New app`, and pick the appropriate repository, branch, and file path.
3. Hit `Deploy`.
...
### What is Streamlit’s [experimental connection](https://docs.streamlit.io/library/api-reference/connections/st.experimental_connection?ref=streamlit.ghost.io) feature?

![](https://streamlit.ghost.io/content/images/2023/08/ChatGPT.png)

![](https://streamlit.ghost.io/content/images/2023/08/LlamaIndexChatbot.png)

![](https://streamlit.ghost.io/content/images/2023/08/no.png)

![](https://streamlit.ghost.io/content/images/2023/08/yes.png)
